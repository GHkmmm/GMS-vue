...
<div id="modalManage" class="modal">
    <div class="modal-content animate">
        ...
        <div class="container">
            ...
            <input type="text" placeholder="请输入场地名字" id="placeName" required>

            <label><b>场地位置</b></label>
            <input type="text" placeholder="请输入场地位置" id="placeLocation" required>

            <button  class="btn btn-outline-primary" @click="addPlace(this.placeName,this.placeLocation)" >确认</button>
...
export default {
 name:"modalManage",
 addPlace(placeName,placeLocation){
     addPlace(this.placeName,this.placeLocation).then(res=>{
         if(res.code ==200){
             this.$toast.suc("添加成功")
         }else{
             this.$toast.err("添加失败")
